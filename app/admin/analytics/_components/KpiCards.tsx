...
export async function KpiCards() {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <KpiCard title="Total Enrollments" value={totalEnrollments.toString()} icon={ShoppingCart} />
      <KpiCard title="Total Users" value={totalUsers.toString()} icon={Users} />
      <KpiCard title="Total Courses" value={totalCourses.toString()} icon={BookOpen} />
    </div>
  );
}
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {Array.from({ length: 4 }).map((_, i) => (
                <Card key={i}>
                    <CardContent className="h-[108px] animate-pulse bg-muted rounded-lg" />
                </Card>
            ))}
        </div>
    );
