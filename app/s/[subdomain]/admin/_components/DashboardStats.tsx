...
export async function DashboardStats({ t }: DashboardStatsProps) {
  ...
  return (
    <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
      <StatCard
        title={t('kpi.totalStudents.title')}
        value={stats.totalCustomers}
        description={t('kpi.totalStudents.description')}
        Icon={Users}
      />
      <StatCard
        title={t('kpi.activeStudents.title')}
        value={stats.activeStudents}
        description={t('kpi.activeStudents.description')}
        Icon={Activity}
      />
      <StatCard
        title={t('kpi.newEnrollments.title')}
        value={stats.newEnrollments}
        description={t('kpi.newEnrollments.description')}
        Icon={ShoppingCart}
      />
      <StatCard
        title={t('kpi.revenueThisMonth.title')}
        value={`$${stats.totalRevenue.toFixed(2)}`}
        description={t('kpi.revenueThisMonth.description')}
        Icon={DollarSign}
      />
    </div>
  );
