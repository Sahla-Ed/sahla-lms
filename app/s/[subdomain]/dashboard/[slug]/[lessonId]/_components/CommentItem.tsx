      ...
      <div className='flex-1 space-y-2'>
        <div className='flex items-center justify-between'>
          <div className='flex items-center gap-2'>
            ...
            {isInstructor && <Badge variant='secondary'>{t('instructorBadge')}</Badge>}
          </div>
        ...
        {!isEditing ? (
          ...
          <div className='space-y-2'>
            ...
            <div className={`flex gap-2 ${isRTL ? 'justify-start' : 'justify-end'}`}>
              <Button variant='ghost' size='sm' onClick={() => setIsEditing(false)} disabled={isEditingPending}>
                {t('cancelButton')}
              </Button>
              <Button size='sm' onClick={handleUpdate} disabled={isEditingPending || !editText.trim()}>
                {isEditingPending && <Loader2 className={cn('h-4 w-4 animate-spin', isRTL ? 'ml-2' : 'mr-2')} />}
                {t('saveButton')}
              </Button>
            </div>
          </div>
            ...
            <>
              ...
              {canEdit && (
                <Button variant='ghost' size='sm' onClick={() => setIsEditing(true)}>
                  <Edit className={cn('h-3 w-3', isRTL ? 'ml-1' : 'mr-1')} /> {t('editButton')}
                </Button>
              )}
              {canDelete && (
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button variant='ghost' size='sm' className='text-destructive hover:text-destructive'>
                      <Trash2 className={cn('h-3 w-3', isRTL ? 'ml-1' : 'mr-1')} /> {t('deleteButton')}
                    </Button>
                  </AlertDialogTrigger>
                  <AlertDialogContent>
                    <AlertDialogHeader>
                    ...
                    <AlertDialogFooter>
                      ...
                      <AlertDialogAction onClick={handleDelete} className='bg-destructive hover:bg-destructive/90' disabled={isDeletingPending}>
                        {isDeletingPending && <Loader2 className='mr-2 h-4 w-4 animate-spin' />}
