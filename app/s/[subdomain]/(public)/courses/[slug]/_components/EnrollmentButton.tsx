...
export function EnrollmentButton({
  ...
  if (isAdmin) {
    return (
      <Button asChild size="lg" className="w-full">
        <Link href={`/admin/courses/${courseId}/edit`} className={cn('flex', { 'flex-row-reverse': isRTL })}>
  ...
  if (isEnrolled) {
    return (
      <Button asChild size="lg" className="w-full">
        <Link href={`/dashboard/${courseSlug}`} className={cn('flex', { 'flex-row-reverse': isRTL })}>
  ...
  return (
    <Button onClick={handleEnroll} disabled={pending} size="lg" className="w-full">
      {pending ? (
        <>
          <Loader2 className={cn('size-4 animate-spin', isRTL ? 'ml-2' : 'mr-2')} />
          {tEnroll('processing')}
        </>
      ) : (
        tEnroll('enrollNow')
